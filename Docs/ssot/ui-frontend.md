...
| Ruling | Date | Content |
|---|---|---|
| **R100 — THE TWO R98 DOORS ARE NOT COLOUR-CODED** | **2026-08-08** | **LEAGUE PLAY and STAKED PLAY differ by DENSITY, MOTION RATE and CONTENT — never by palette.** Electric `#1E5AFF` leads fill on both doors; Arc-Violet `#A855F7` stays a rim/glow/focus accent on both and never touches core, fill or text. **This confirms what §10.2 already requires:** CHROME is the app's own furniture and is identical for every player, while IDENTITY is what a player picked or a brand owns and is resolved by tag through the registry. Giving the staked door its own hue would make chrome carry meaning that belongs to identity — the failure §10.2 names as *invisible on any single screen: it only shows up as the same colour meaning two different things in two places.* **Closes the one derived-not-ruled item in `IRONICS_HOME_SCREEN_SPEC.md` §2**, which was the last design blocker on building the home screen. The two remaining §9 items are ART dependencies, not design ones: the type ramp is unapproved and the bolt lockup integrates as produced. **The door STRUCTURE is unblocked and buildable now.** |
