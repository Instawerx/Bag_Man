...
- **Cosmetic axes** — independent, composable: color × finish × beam × **facemask** (all proven). New parts add **new axes or slots** — but note **visors are NOT a new slot: they ARE the proven Facemask axis** (`FacemaskId` → `RefreshFacemaskForPawn`); a helmet would be the new slot. Same matrix logic, more dimensions.
...
| 11 | **Skins — Enhanced Designs, Boost Amounts** + Economy | cosmetic axes + rarity SSOT | enhanced-design axis + boost values |
