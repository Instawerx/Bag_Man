...
| 41 | **CC-X41 — the fixed-wrist-side path is live but no row exercises it** | RECORDED 2026-08-24. The operator flagged this as `EAFLWearSlot::WristLeft/WristRight` declared and used by no row; CORRECTED ON READ: `EAFLWearSlot` has no such members -- it is `Unset/Neck/Pendant/Wrist`, collapsed to a single `Wrist` when the either-side mechanism was built. The real value the concern points at is **`EAFLWristSide::Left/Right` + `FAFLCatalogEntry::WristSideWhenFixed` + the `!bWristEitherSide` branch** of `ResolveWearableSlot` (`AFLCosmeticLoadoutComponent.cpp:1147`). **It is NOT silently ignored** -- the resolver honours it: a row with `bWristEitherSide=false` resolves to its `WristSideWhenFixed` side and refuses if that wrist is full. What is true is that **no current row selects it**: all four wrist rows carry `bWristEitherSide=true`, because the symmetry measurement (2026-08-24, ~2% residual vs 14-28% on the odd axis) confirmed all four meshes are mirror-symmetric and fit either wrist. **KEPT DELIBERATELY, not dead code**: it is the operator's own ruled fallback for the day Tripo returns a handed single mesh -- *"handed singles mean watches right, bracelets left and the flag goes false on those rows."* Unexercised-by-data is the correct state while all art comes back symmetric; it becomes exercised the first time a handed piece ships. Not removed, because removing the ruled fallback would mean re-adding it under pressure the day it is needed. |
| 42 | **CC-X42 — the gen pipeline lets a canary and a product share an output path** | RECORDED 2026-08-24. `tripo_generate.py` writes `<name>.glb` under `Tools/Generated/`, and a text-prompt CANARY and the real image-gen PRODUCT for the same piece land at the SAME path. `SK_BagMan_Chain_FoundersPurps` was the text-prompt canary in the UE project for some time and passed every structural check -- bone count, socket on the lowest bone, vert count -- because those verify SHAPE, not PROVENANCE. It was overwritten by its real image-gen GLB by luck (the batch reused the name), not by design. **A canary that passes every check is exactly the CC-X37/value-vs-provenance shape**: the instrument cannot tell a stand-in from the thing it stands in for. FIX (owed): namespace canary output separately -- e.g. `Tools/Generated/_canary/` -- so a canary can never occupy a product path, and a product asset carries a provenance marker (source image hash or task id) that a check can read. Until then `SK_BagMan_Chain_FoundersPurps` is STILL the canary (its real mesh is flat -- see REQUIREMENT 0) and MUST NOT SHIP; the re-shoot replaces it. |
| 32 | **CC-X37 — the accessory axis is built, correct, and unreachable** | **RECORDED 2026-08-23, NOT A DEFECT IN WHAT EXISTS.** `FAFLAccessorySet` is authored, replicated, server-authoritative and refuses correctly on every path — unowned, slotless, pendant-without-chain, wrist-with-no-open-side — and `afl.Test.Wearables` passes 9 of 9 in PIE with an ownership control. **Nothing reads it to attach a mesh.** A repo-wide sweep finds `AccessorySet` and `ResolveSocket` referenced ONLY in `AFLCosmeticLoadoutComponent.cpp` (the writer) and one comment; there is no consumer on the pawn, no spawn-time read, no attach. The seven `accessory_*` sockets exist on `SK_Mannequin` and nothing ever attaches to them.<br><br>**SO A PURCHASED PIECE TODAY IS OWNED, LISTED, AND RESOLVES TO A SLOT — AND DOES NOT RENDER.** That is two separate gaps, and the art landing closes only one: Tripo meshes give the consumer something to attach, but the consumer itself is unwritten work. Neither is scheduled here.<br><br>**THIRD INSTANCE OF BUILT-CORRECT-UNREACHABLE THIS PROGRAMME**, after **CC-X21** (four catalog fields exposed to Blueprint with 0 links anywhere in 200 Blueprints) and **CC-X32** (10 priced duplicate colour rows the loadout's axis filter can never surface). The shared shape is that every local test passes: the writer is correct, the data is correct, the refusals are correct, and the thing is still not reachable from where a player stands. A proof that ends at the seam it was written for does not notice that nothing is on the other side. |
