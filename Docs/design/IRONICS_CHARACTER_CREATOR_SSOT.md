...
| 13 | **CC-X21** — inert-but-exposed catalog fields | **STILL OPEN 2026-08-21, and the scripted route is EXHAUSTED.** A Blueprint-graph walk cannot answer it: `UbergraphPages`, `FunctionGraphs`, `EventGraphs` and `MacroGraphs` are all **protected and unreadable from the Python API**. The probe reported that rather than returning “no reads found” — an API that cannot see nodes reporting zero nodes is a confident null. **Disk evidence, which is weaker than it looks:** `AFLW_Menu_CosmeticShop.uasset` CONTAINS the strings `MintCap` and `bIntactOnlyBundle`. That proves the NAME is serialised in the asset, NOT that a node reads it — it could be a struct member, a stale pin, or a dead reference. **Two ways to close:** (a) the 15-second Find-in-Blueprints with an editor open, as originally scoped; or (b) a falsification — drop `BlueprintReadOnly` from the four fields and recompile: if the widget breaks, something reads them, and if nothing breaks the exposure was never used and removing it IS the fix. Risk unchanged: cosmetic only, price is server-read on both purchase paths. | `MintCap`, `Tier`, `ContentTier`, `bIntactOnlyBundle` are inert in C++ yet `BlueprintReadOnly`, so a Blueprint could be reading them. **Cosmetic risk only — price is server-read on BOTH purchase paths.** OPEN: Find-in-Blueprints for `MintCap`, and Ctrl+F it in `AFLW_Menu_CosmeticShop`. Fifteen seconds while an editor is open. |
