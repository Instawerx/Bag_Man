...
| 30 | **CC-X33 — `pelvis` / `spine_03` solver behaviour unreadable** | **MEASURED 2026-08-21, HELD by operator ruling.** Both sit OUTSIDE every goal-driven chain: `IK_ProMod_FBIK` has 28 retarget chains and exactly **4** carry a goal (`LeftLeg` thigh_l→ball_l, `LeftArm` upperarm_l→hand_l, `RightLeg`, `RightArm`). `pelvis` is in no chain; `spine_03` is in chain `Spine` with `goal=None`. **But a full-body solver can propagate into spine/pelvis to reach its goals, and that setting could NOT be read:** `get_solver_at_index` returns unreadable and `read_graph` on `CR_ProMod_FBIK` returns **0 nodes** — the rig's VM graph is not reachable. **Settling it needs the IK Rig editor, or a PIE observation of bone drift under a reaching goal.** ⚠ NOTE the drift concern is NARROWER than first framed: accessories attach via `SetupAttachment(component, SocketName)` and inherit the socket transform POST-SOLVE, so a solver-driven bone moves an attached accessory WITH the body — correct for something worn. Drift would require READING a pre-solve transform, and nothing in the cosmetics path does. |
| 31 | **CC-X34 — socket authoring is not scriptable** | **MEASURED 2026-08-21.** `accessory_head` / `accessory_clavicle_l` / `accessory_clavicle_r` on `SK_Mannequin` were ruled and could NOT be authored: `SkeletalMeshSocket::SocketName` is **read-only** from Python and the skeleton's `Sockets` array is **protected**. Verified by read-back — 170 socket names before, 170 after, zero registered. Needs the Skeleton editor UI or a C++/commandlet path. The axis and attach path do NOT depend on it and shipped without it. |
