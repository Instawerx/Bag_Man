...
| 12 | **CC-X20** — type-lint residue | **ANSWERED 2026-08-21.** The **50 unmapped** are the `AFL.WeaponSkin.NeonCamo.*` rows: weapon SKINS carrying `Type=WEAPON`, a (type, prefix) pair the lint's table has no rule for. The **10 `AFL.Body.` mismatches are REAL rows** — all `Type=FINISH`, `Acquisition=Direct`, ids using `AFL.Body.` where the convention implies `AFL.Finish.`. **These 50 are the SAME rows as CC-X31**: 177 WEAPON rows = 127 linked (98 hand cannon + 29 pool, all `AFL.Weapon.`-prefixed) + 50 unlinked, and the 50 unlinked are exactly the 50 mis-prefixed. A skin correctly has no equipment item def; the defect, if any, is the **Type**, not the missing link. REMAINING (convention, operator's call): should the 50 be `WEAPON_ACCESSORY`, should `AFL.Body.` be renamed to `AFL.Finish.`, and should the lint gain rules for these pairs? ⚠ The re-derivation used an ASSUMED convention table, not the lint's own, so its raw totals differ from the lint's `mismatch=15 unmapped=50` by construction — it identifies WHAT the rows are, it does not restate the lint's arithmetic. | `TYPELINT checked=427 mismatch=15 (AFL.Character.=5, AFL.Body.=10) unmapped=50`, none in the lint's known-positives list. **No `AFL.Body` row was touched by the roster cut, so these predate CC-6.3.** OPEN: determine what the 50 unmapped are, and whether the 10 `AFL.Body.` mismatches are real. |
| 13 | **CC-X21** — inert-but-exposed catalog fields | **CC-X21 CLOSED 2026-08-21 — handed to the manual check.** `MintCap`, `Tier`, `ContentTier`, `bIntactOnlyBundle` are inert in C++ yet `BlueprintReadOnly`. **RESIDUAL ACTION (15 seconds, with an editor already open):** Find-in-Blueprints for `MintCap`; if `AFLW_Menu_CosmeticShop` has a live get-node, implement the field or give the UI a real accessor — if it does not, drop `BlueprintReadOnly` from all four and the hazard is gone. **RISK IS BOUNDED AND UNCHANGED: cosmetic only.** Price is server-read on BOTH purchase paths, so the worst case is a UI showing a meaningless number, never a mispriced sale. **THE SCRIPTED ROUTE IS EXHAUSTED — do not spend more on it.** (a) A Blueprint-graph walk cannot work: `UbergraphPages`, `FunctionGraphs`, `EventGraphs`, `MacroGraphs` are all **protected and unreadable from the Python API**. The probe reported that rather than “no reads found”, since an API that cannot see nodes reporting zero nodes is a confident null. (b) The remove-exposure falsification was attempted TWICE and returned no information at either attempt: the first hung 1h42m on a stale UBT mutex (a foreground build killed at a timeout orphaned the lock, and `-WaitMutex` then blocked forever); the second was killed by a system graphics crash mid-build, plausibly triggered by 11+ parallel compile workers on a 16 GB machine. Both left the tree recoverable and were restored byte-for-byte, but the cost far exceeded a cosmetic-only item's value. **⚠ CORRECTION — the disk evidence is worth NOTHING, not merely “weak”:** `AFLW_Menu_CosmeticShop.uasset` contains EVERY field name in the struct — `CosmeticId`, `PriceVolts`, `bTransactable`, all of them — because the widget references the `FAFLCatalogEntry` TYPE and the whole member-name table is serialised. “The asset contains `MintCap`” is not partial evidence of a read; it is no evidence at all. |
...
| 15 | **CC-X27** — packaged loadout reverted while the store stayed current | **CANNOT CONFIRM 2026-08-21 — no reproducible symptom on record.** Nothing in the repo describes the revert, and there is no stated repro to run against. RECOMMEND CLOSING as obsolete unless the operator can name the symptom; re-open with a repro if it recurs. Recording the inability to confirm rather than quietly dropping it. | Operator notes the loadout was changing anyway. **Lowest priority, likely obsolete.** OPEN only to confirm or close. |
...
| 26 | **CC-X31 — 50 WEAPON rows carry no `ItemDefClass`** | **MEASURED then RESOLVED 2026-08-21.** → **They are the 50 `AFL.WeaponSkin.NeonCamo.*` rows** (see CC-X20): weapon skins typed `WEAPON`. A skin has no equipment item def because it is not equipment, so the missing `ItemDefClass` is CORRECT and the open question is the **Type**, which merges into CC-X20's convention call. The “possibly colourway variants deferring to a base def” reading was right. Original measurement kept below.  Of 428 catalog rows, 301 carry an empty `ItemDefClass`. For BEAM (49), FINISH (44), SKIN_COLOR_EDGE (42) and FACEMASK (38) that is correct — they are visual, not inventory items. But **50 are `WEAPON` type**: rows that can be owned but resolve to no equipment. This is the MIRROR IMAGE of READ C's 31 (item defs with no row); these are rows with no item def. **Two readings, and I have not distinguished them:** they may be colourway variants deliberately deferring to a base def's equipment, or a genuine hole where ownership grants nothing. Deciding by name would be exactly the mistake this programme keeps catching — it needs a pass that resolves what each row is expected to equip. Distinct from CC-X22: these rows are not currently sellable, so there is no shipping consequence today. Separately from the 50, the 49 `.XT` rows also read empty — that one is DELIBERATE (`149948f3` cleared the link during the bundle conversion) and is not part of this item. |
