...
| **I-28** | **Preview stage rulings (all delivered + RT-verified; wheels operator-watched "all wheels AAA").** (a) FIVE colour wheels: Body, Edge, Glow, Visor, **Emblem** (new channel → chest-decal NeonColor); a channel that is dead on a chassis shows **disabled with a reason**, never a live wheel writing to a dead parameter (ProMod: Body inert, Edge rimless). (b) No platform disc; the character stands **on the floor**, grounding MEASURED from worn-body bounds. (c) Pose = **relaxed unarmed idle** (MM_Unarmed_Idle_Ready looping on the driving mesh). (d) **Open stage**: no kiosk capsule / halo ring / backdrop slab around the character — out front and clear against the neon key light + gradient dome + electric-arc element (applies to the placed AFL_ArmoryPod too, same class). (e) Camera **auto-frames** from measured bounds; body yaw squares the visor + chest emblem to the lens (afl.Loadout.BodyYaw=225) | operator chat rulings 2026-08-28 · commits c78532e2…6bbfd8bd |
| **I-29** | **Loadout/creator refinement batch (all delivered + probe-proven P20–P46; operator live-watched "works AAA" per item 2026-08-29).** (a) The character preview lives **inside the detail card** (ScaleBox, aspect-safe); the floating world-side box is gone — nothing stacks or clips. (b) The browsed weapon renders **in the character's hands** through a display-only equip rail (a controller-less preview pawn has no Inventory/QuickBar, so the one QuickBar rail structurally cannot arm it); replace-by-sweep, and the **akimbo hand-cannon pair arms both forearms**. (c) **Two-slot weapon cache** boxes mirror the committed main/left mounts (EQUIP fills 1, SWAP exchanges). (d) 127/127 weapon SKU **ShopThumbnails** rendered + assigned (continuous-capture rig laws banked); owned grid 3-col with SELECTED pill + ellipsised names; nine filter-pill tabs fit the band. (e) Creator **EQUIP verb** (I-13 gap closed: CreatorCommitWorking through the one RPC), token E-field, **compact thumbnailed part-picker tile** for the 420 column. (f) The electric-arc element is **REMOVED** from the open stage (supersedes the I-28(d) arc clause). (g) **Placed kiosk pawns self-dress** from the local committed selection (the bare-grey gap: the look lives entirely in the runtime skin fan-out). (h) The greybox ActionBar (orphaned, zero referencers) is **retired**, not styled | operator chat rulings 2026-08-29 · commits ce7ba39d…7c53d547 |
