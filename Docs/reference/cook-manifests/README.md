...
⭐ **`cook_20260810f` is the current reference baseline** — the most recent clean cook from a healthy
...
**What to do, regardless of root cause:**
1. **Diff every cook against a manifest.** None of this appears in the totals, the warnings, or the exit
   code — only in the package-level diff.
2. **Ship from clean cooks only**; an incremental tree emits packages a clean one does not.
3. The durable fix is to stop leaving it to chance — force-cook the DataLayer directories, the same
   remedy `DirectoriesToAlwaysCook` already applies to five other classes of silently-missing asset.
   Tracked on the ticket, with the measured-cost requirement that block imposes on itself.
