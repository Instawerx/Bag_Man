...
- **PRICING PASS -- general-catalog ECONOMY CLOSED `2026-07-10`:** the approved cheap-first table applied + disk-verified + wallet-verified + committed. **Weapons (13):** 10 FREE (`GrantedFree` base roster) + **3 Impulse** ($0.99 = 990 V / 9,900 W pay-either: Arclight/Voltaic/Ioncaster, the custom-mesh pilot signatures). **Skins (50):** **7 FREE** base-palette (6 NeonCamo flagships + newly-authored **Black** MI+row) + 30 SPARK Impulse + 13 SURGE **Premium** (7,990 V Volts-only). **Beams (49):** **6 FREE** base-palette (no Black beam -- 7-skin/6-beam asymmetry intentional) + 30 SPARK Impulse + 13 SURGE Premium. Wallet loop reads it verbatim (`GrantedFree` auto-owns + shop-excluded; Impulse pay-either; Premium **Volts-only wall enforced at the purchase layer** `AFLWalletComponent.cpp:342-344`, not just display). Mostly-free base roster + exclusives premium, **no B2 dep** (grails = separate premium track). Black NeonCamo skin authored (gunmetal ramp off the locked master `M_AFL_WeaponSkin_NeonCamo`) to complete the 7-color free starter (E2 Black-edge precedent).
...
| WEAPON | 13 base | 10 FREE / 3 Impulse | `AFL.Weapon.<W>` base rows: **10 GrantedFree** (Tempest/Vanguard/Breacher/GTM/PP9/N90/Remore/DE42/Judge45/NFP16) + **3 Impulse $0.99** (Arclight/Voltaic/Ioncaster, pilot signatures), priced 2026-07-10. Per-weapon `.<Color>` grid REMOVED 2026-07-10 (149 rows). Colors = universal `AFL.WeaponSkin.*` (**50** w/ Black: 7 free + 30 Impulse + 13 Premium) + `AFL.Beam.*` (49: 6 free + 30 Impulse + 13 Premium). |
