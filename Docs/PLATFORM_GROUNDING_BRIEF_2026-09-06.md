...
## 0.5 Operator rulings (2026-09-06, after reading this brief) - PLAN OF RECORD

Operator's words: *"DeepWaterStation+CyberPunk (323 MB, ~170 after 4K cap) are not used anywhere in our game they are likely excess content packs never used and not needed. As downloads are working for now we need to keep any scaling under $50 a month. Most important is Admin controls and Dashboards UI and UX. Then a content and Roadmap pass. Everything else approved so far remaining in our constraints and meeting our Quality levels."*

| # | Ruling | Effect on the decision list |
|---|---|---|
| R1 | **Cull DeepWaterStation + CyberPunk** (excess packs, never used) - **BLOCKED 2026-09-06 by the reference audit (`Docs/dist/CONTENT_PACK_CULL_AUDIT_2026-09-06.md`): both packs are hard-imported by the shipped ARCANEON venue (L_Arena_04 + B_ARCANEON_Structure/Dressing), 60 cooked packages = 322.8 MB compressed = 8.3 %% of the 0.1.3 download; a root NeverCook/git rm today reproduces the 0.1.2 cook failure on client AND server. Re-ruling needed: options A (slice migration, 0 download bytes), B (re-dress ARCANEON, -322.8 MB, touches proven art), C (4K cap on the slice, est. -120..-170 MB), D (hygiene only).** | Enters the next release's build via NeverCook + reference audit + cook verification (0.1.2 law: verify by cook, not manifest). Repo removal is a separate destructive step. Answers part of #18 (Tier 1B cull items). |
...
| R2b | **Live data footprint read 2026-09-06 (DynamoDB item counts, read-only)** | Portal: IronicsAccounts 1 · IronicsAdmins 1 (the operator) · IronicsApplications 1 · IronicsEntitlements 1 · IronicsAuditLog 3 · IronicsCounters 1 · IronicsProviderLinks 2 · IronicsRefreshTokens 15 · ClaimCodes/UnattributedPayments/RateLimits 0. Game backend (bagman-*): match-escrow 16 · match-settlement-ledger 8 · bundle-mint-ledger 39 · queue-ticket-index 38 · player-ratings 2 · rating-ledger 7 · counted-entitlements 7 · conditional-entitlements 4 · dm-messages 4 · creator-builds 1 · kill-telemetry 0 · presence 0. **Analytics stack reality:** Resend (magic-link mail) is wired; **PostHog, Sentry, Loops are NOT present anywhere in Ironics-Platform** (grep) -> GTM §6 is ruled but un-built; #28 is "build it", not "choose it". |
...
2. [ ] ADMIN BASELINE: Was apps/api/scripts/seed-first-admin.ts ever run against prod? If not, may it be run for your account (01M06WNV71GP2QWYPBQA5JT4WC) before any admin work starts? **[ANSWERED 2026-09-06 by a read of DynamoDB `IronicsAdmins`: exactly one row, accountId 01M06WNV71GP2QWYPBQA5JT4WC -> seed-first-admin WAS run. Nothing to do.]**
